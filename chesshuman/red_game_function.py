...
def red_attack_judge(s):
	...
	s.red_attack_judge += 1
	#print(s.red_attack)
	#print(s.red_attack_judge)
	#print(s.red_attack_time)
	#print('++++')
...
def rival_chessman_move(client, s):
	...
	if s.order == -1 and s.get == 1:
		for key in s.black_label.keys():
			if s.srcx == s.black_label[key]['cur_loc'][0] and s.srcy == s.black_label[key]['cur_loc'][1]:
				s.black_backup = [key, s.black_label[key]]
	if s.black_backup:
		#改为落子状态
		s.running = False
	#更改黑棋位置
	if s.black_backup and s.get == 1:
		if loc_judge(s.dstx, s.dsty, s) == 1:
			#删除选中位置的棋子
			for key in list(s.red_label.keys()):
				if [s.dstx, s.dsty] == s.red_label[key]['cur_loc']:
					del s.red_label[key]
		s.black_label[s.black_backup[0]] = s.black_backup[1]
		s.black_label[s.black_backup[0]]['cur_loc'] = [s.dstx, s.dsty]
	...
	if event.button == 1 and 47 < event.pos[0] < 853 and 47 < event.pos[1] < 943:
		...
		if s.running:
			...
			if s.order == 1:
				for key in s.red_label.keys():
					...
					+ (s.red_label[key]['cur_loc'][1] - y) ** 2) < s.r:
		...
		#鼠标再次按下，落下棋子
			...
			if s.r < event.pos[0] < 810 + s.r and s.r < event.pos[1] < 900 + s.r:
				...
				if s.red_backup :
					#判断所走位置不符合中国象棋规则
					if [x, y] not in s.red_label[s.red_backup[0]]['nxt_loc'] or loc_judge(x, y, s) == 1:
					...
					#落子在新的位置
						...
						if loc_judge(x, y, s) == -1:
							...
							#位置被新棋子占领
							s.red_label[s.red_backup[0]] = s.red_backup[1]
							s.red_label[s.red_backup[0]]['cur_loc'] = [x, y]
...
def loc_to_move(s):
	...
	for key in s.red_label.keys():
		...
		while i < len(s.red_label[key]['nxt_loc']):
			...
			#去掉相越过楚河汉界的走位
			if key == '相' or key == '相2':
				if s.red_label[key]['nxt_loc'][i][1] < 540:
					s.red_label[key]['nxt_loc'].pop(i)
					i = i - 1
			i = i + 1
		#蹩马腿
	...
	for key in s.black_label.keys():
		...
		while i < len(s.black_label[key]['nxt_loc']):
			...
			#去掉象越过楚河汉界的走位
			if key == '象' or key == '象2':
				if s.black_label[key]['nxt_loc'][i][1] > 450:
					s.black_label[key]['nxt_loc'].pop(i)
					i = i - 1
			i = i + 1
		#蹩马腿
